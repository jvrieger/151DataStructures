...
/*
 * Map implementation using hash table with double hashing.
 *
 * @author Eric Zamore
...
public class DoubleHashMap<K,V> extends AbstractHashMap<K,V> {
    ...
    private int insertCalled = 0;
    public static final int LOWER_CAPACITY = 999983; // 0 < LOWER_CAPACITY < capacity - 1
  ...
  public DoubleHashMap(int cap, int p) { super(cap, p); }
	...
	return (double) totalProbes/ (double) insertCalled;
    }
  ...
  private int findSlot(int h, K k) {
    ...
    int i = 0;
    int j = h;
    probes = 1;                                     // index while scanning table
    do {
      ...
      j = (j + hashValue2(k)) % capacity;         // keep looking (cyclically) by incrementing i
      probes++;
      i++;
    } while (i < capacity);
    this.totalProbes += probes;
    if(probes > maxProbes) {maxProbes = probes;}  // stop if we return to the start
